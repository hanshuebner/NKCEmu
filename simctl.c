...
static int load_eeprom(int, char *);
...
static int do_getfile(char *s)
{
	...
	*pfn = '\0';
	...
	if (*s == ',')
		...
		wrk_ram	= NULL;
    /* if -b option is set, file is a binary eeprom oontent with start address 0x0000*/
    if (b_flag==1)
    {
...
/*
 *  Loader for binary images of EEPROM content.
 *  It is assumed, that the file content is destined
 *  for the starting address 0x0000
 */
static int load_eeprom(int fd, char *fn)
{
    BYTE fileb[3];
    unsigned count, readed;
    int rc = 0;
...
static void do_help(void)
{
    ...
    puts("e filename[,address]      read EEPROM-dump into memory");
