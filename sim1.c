...
static int getDiffMillies(struct timeval *t1, struct timeval *t2);
...
void cpu(void)
{
	...
	do {
        ...
        gettimeofday(&akttime,NULL);
        diff=getDiffMillies(&oldtime,&akttime);
        if (diff>20)
        {
            if (gdp64_set_vsync(1)==0)
            {
                oldtime.tv_sec=akttime.tv_sec;
                oldtime.tv_usec=akttime.tv_usec;
            }
            else
            {
                gettimeofday(&akttime,NULL);
...
static int getDiffMillies(struct timeval *t1, struct timeval *t2)
{
    ...
    {
        ms=(t2->tv_sec-t1->tv_sec)*1000+(1000000-t1->tv_usec+t2->tv_usec)/1000;
    }
    else
    {
